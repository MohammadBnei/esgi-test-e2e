...
describe("Home Page Firefox", function() {

	before(function () {
		return firefoxBrowser.get(serverUri)
			.then(() => firefoxBrowser.wait(webdriver.until.elementLocated(webdriver.By.css('main'), 5000)))
	});

	after(function() {
		return firefoxBrowser.quit();
	});
	 ...
	 */
	it("Should load the home page and get title", async function() {
			const title = await firefoxBrowser.getTitle()
			assert.strictEqual(title, appTitle);
	});
	
	it("Should load the articles", async function() {
			const articlesCount = await firefoxBrowser.findElements(cssLocator('li'))
			assert.strictEqual(articlesCount.length, 6)
	});

	it("Should add an article to the cart", async function() {
		const firstArticle = await firefoxBrowser.findElement(cssLocator('.product-price'))
		let articlePrice = await firstArticle.findElement(cssLocator('div'))		
		const articleBtn = await firstArticle.findElement(cssLocator('button'))
		await articleBtn.click()

		await firefoxBrowser.wait(webdriver.until.elementLocated(cssLocator('div.total>div'), 2000))

		const total = await firefoxBrowser.findElement(cssLocator('div.total>div'))
	...
	it("Should proceed to checkout", async function() {
		const total = await firefoxBrowser.findElement(cssLocator('div.total'))
		const proceedBtn = await total.findElement(cssLocator('button'))
	...
	it("Should checkout", async function() {
		...
		const email = await firefoxBrowser.findElement(webdriver.By.name('email'))
		const name = await firefoxBrowser.findElement(webdriver.By.name('name'))
		const address = await firefoxBrowser.findElement(webdriver.By.name('address'))

		await firefoxBrowser.wait(webdriver.until.elementLocated(webdriver.By.id('checkoutBtn'), 1500))

		const checkoutBtn = await firefoxBrowser.findElement(webdriver.By.id('checkoutBtn'))
		...
		await firefoxBrowser.wait(webdriver.until.elementLocated(cssLocator('div.main'), 2000))

		let success = await firefoxBrowser.findElement(cssLocator('div.main'))
		success = await success.getText()
